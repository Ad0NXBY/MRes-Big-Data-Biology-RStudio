...
# Save unfiltered DEG list as a .csv file for GSVA
write.csv(
  results_plenti_vs_KO22, 
  file.path(data_dir, "DEG_plenti_vs_KO22_unfiltered.csv"), 
  row.names = TRUE
)
...
write.csv(
  ...
  file.path(data_dir, "DEG_plenti_vs_KO23_unfiltered.csv"), 
  row.names = TRUE
)
...
Volcano_Plot_Plenti_v_KO23 <- ggplot(volcano_data_23, aes(x = log2FoldChange, y = -log10(padj), color = Significance, label = Gene)) +
  ...
  geom_text(data = top_downregulated_23, aes(label = GeneSymbol), vjust = 1, hjust = 1, size = 3, color = "bl", check_overlap = TRUE) +
