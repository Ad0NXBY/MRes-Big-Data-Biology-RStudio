...
# Resolve duplicate symbols (keep the most significant entry)
ranked_list_KO22 <- results_plenti_vs_KO22_clean %>%
  dplyr::group_by(GeneSymbol) %>%
  dplyr::slice_max(ranking_metric) %>%  # Keep highest ranking entry per symbol
  dplyr::ungroup() %>%
  dplyr::arrange(desc(ranking_metric)) %>%  # Sort descending
  dplyr::select(GeneSymbol, ranking_metric)
