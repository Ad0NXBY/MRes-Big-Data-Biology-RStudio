...
write.table(Counts_gene2, file.path(data_dir, "merged_featurecounts2.txt"), sep = "/t", quote = FALSE, row.names = FALSE)
...
ranked_list_KO22 <- results_plenti_vs_KO22_clean %>%
  group_by(GeneSymbol) %>%
  arrange(padj, desc(abs(log2FoldChange))) %>% 
  slice(1) %>%  # keep the top row per gene
  ungroup() %>%
...
write.table(
  ranked_list_KO22,
  file.path(data_dir, "GSEA_ranked_list_KO22_padj.rnk"),
  sep = "\t",
  col.names = FALSE,
  row.names = TRUE,
  quote = FALSE
...
#Prepare ranked list for GSEA
ranked_list_KO23 <- results_plenti_vs_KO23_clean %>%
  group_by(GeneSymbol) %>%
  arrange(padj, desc(abs(log2FoldChange))) %>% 
  slice(1) %>%  # keep the top row per gene
  ungroup() %>%
...
write.table(
  ranked_list_KO23,
  file.path(data_dir, "GSEA_ranked_list_KO23_padj.rnk"),
  sep = "\t",
  col.names = FALSE,
  row.names = TRUE,
  quote = FALSE
...
Volcano_Plot_Plenti_v_KO22 <- ggplot(volcano_data_22, aes(x = log2FoldChange, y = -log10(padj), color = Significance, label = Gene)) +
  ...
  geom_text(data = top_upregulated_22, aes(label = GeneSymbol), vjust = 1, hjust = 1, size = 3, color = "black", check_overlap = TRUE) + #Red for upregulated
  geom_text(data = top_downregulated_22, aes(label = GeneSymbol), vjust = 1, hjust = 1, size = 3, color = "black", check_overlap = TRUE) + #Blue for downregulated
...
Volcano_Plot_Plenti_v_KO23 <- ggplot(volcano_data_23, aes(x = log2FoldChange, y = -log10(padj), color = Significance, label = Gene)) +
  ...
  geom_text(data = top_upregulated_23, aes(label = GeneSymbol), vjust = 1, hjust = 1, size = 3, color = "black", check_overlap = TRUE) +
  geom_text(data = top_downregulated_23, aes(label = GeneSymbol), vjust = 1, hjust = 1, size = 3, color = "black", check_overlap = TRUE) +
...
write.csv(result.enrich.go.up.bp_KO22, file.path(data_dir, "GO_enrichment_upregulated_BP_KO22.csv"), row.names = FALSE)
write.csv(result.enrich.go.up.cc_KO22, file.path(data_dir,"GO_enrichment_upregulated_CC_KO22.csv"), row.names = FALSE)
write.csv(result.enrich.go.up.mf_KO22, file.path(data_dir,"GO_enrichment_upregulated_MF_KO22.csv"), row.names = FALSE)

write.csv(result.enrich.go.down.bp_KO22, file.path(data_dir,"GO_enrichment_downregulated_BP_KO22.csv"), row.names = FALSE)
write.csv(result.enrich.go.down.cc_KO22, file.path(data_dir,"GO_enrichment_downregulated_CC_KO22.csv"), row.names = FALSE)
write.csv(result.enrich.go.down.mf_KO22, file.path(data_dir,"GO_enrichment_downregulated_MF_KO22.csv"), row.names = FALSE)
...
up_GO_dotplot_bp_KO22 <- ggplot(top_15_up_bp_KO22, aes(x = GeneRatio, y = reorder(Description, GeneRatio), color = p.adjust, size = Count)) +
  ...
  theme_minimal()
ggsave(file.path(plot_dir,"GO_upregulated_dotplot_BP_KO22.png"), plot = up_GO_dotplot_bp_KO22, width = 10, height = 6)
...
up_GO_dotplot_cc_KO22 <- ggplot(top_15_up_cc_KO22, aes(x = GeneRatio, y = reorder(Description, GeneRatio), color = p.adjust, size = Count)) +
  ...
  theme_minimal()
ggsave(file.path(plot_dir,"GO_upregulated_dotplot_CC_KO22.png"), plot = up_GO_dotplot_cc_KO22, width = 10, height = 6)
...
up_GO_dotplot_mf_KO22 <- ggplot(top_15_up_mf_KO22, aes(x = GeneRatio, y = reorder(Description, GeneRatio), color = p.adjust, size = Count)) +
  ...
  theme_minimal()
ggsave(file.path(plot_dir,"GO_upregulated_dotplot_MF_KO22.png"), plot = up_GO_dotplot_mf_KO22, width = 10, height = 6)
...
down_GO_dotplot_bp_KO22 <- ggplot(top_15_down_bp_KO22, aes(x = GeneRatio, y = reorder(Description, GeneRatio), color = p.adjust, size = Count)) +
  ...
  theme_minimal()
ggsave(file.path(plot_dir,"GO_downregulated_dotplot_BP_KO22.png"), plot = down_GO_dotplot_bp_KO22, width = 10, height = 6)
...
down_GO_dotplot_cc_KO22 <- ggplot(top_15_down_cc_KO22, aes(x = GeneRatio, y = reorder(Description, GeneRatio), color = p.adjust, size = Count)) +
  ...
  theme_minimal()
ggsave(file.path(plot_dir,"GO_downregulated_dotplot_CC_KO22.png"), plot = down_GO_dotplot_cc_KO22, width = 10, height = 6)
...
down_GO_dotplot_mf_KO22 <- ggplot(top_15_down_mf_KO22, aes(x = GeneRatio, y = reorder(Description, GeneRatio), color = p.adjust, size = Count)) +
  ...
  theme_minimal()
ggsave(file.path(plot_dir,"GO_downregulated_dotplot_MF_KO22.png"), plot = down_GO_dotplot_mf_KO22, width = 10, height = 6)
...
write.csv(result.enrich.go.down.bp_KO23, file.path(data_dir,"GO_enrichment_downregulated_BP_KO23.csv"), row.names = FALSE)
